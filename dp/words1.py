...
def equals(A, sorted_string_A):
    return ''.join(sorted(''.join(A))) == sorted_string_A


def remaining(words, phrase):
    for word in words:
        for L in word:
            index = phrase.rfind(L)
            if index >= 0:
...
def anagrams(phrase, shortest=2):
    ...
    result = set()
    f_ana(tuple(), phrase, result)
    return result


def f_ana(visited, remaining, result):
    words = find_words(remaining)
    if not remaining:
        result.add(' '.join(sorted(visited)))
    for w in words:
        f_ana(visited + tuple([w]), removed(remaining, w), result)
