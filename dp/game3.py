...
def strategy_d(state):
    "Hold at 30 pending.  Always accept; never double."
    (p, me, you, pending, double) = state
    return ('accept' if double == 'double' else
            'hold' if (pending >= 30 or me + pending >= goal) else
           # 'double' if (pending < 20) else
            'roll')
...
def Q_pig(state, action, Pwin):
    ...
    (p, me, you, pending, double) = state
    if action == 'hold':
        return 1 - Pwin((other[p], you, me+pending, 0, double))
    if action == 'roll':
        return (1 - Pwin((other[p], you, me+1, 0, double))
                + sum(Pwin((p, me, you, pending+d, double)) for d in (2,3,4,5,6))) / 6.
    if action == 'double':
        return 1 - Pwin((other[p], you, me, pending, 'double'))
    if action == 'decline':
        return 0
    if action == 'accept':
        return 1 - Pwin((other[p], you, me, pending, 2))
    else:
        return 0
