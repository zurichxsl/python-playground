...
def do(action, state):
    ...
    if card == 'F':
        cards = 'F'*(n_fox-1) + 'H'*n_hen
    else:
        cards = 'F'*n_fox + 'H'*(n_hen-1)
...
#    score, yard, cards = state
#    n_fox = cards.count('F')
#    p_fox = float(n_fox) / len(cards)
#    win_gather = None
#    win_wait = None
#    actions = avail_actions(state)
#    for action in actions:
#        if 'gather' == action:
#            win_gather = 0
#        if 'wait' == action:
#            win_wait = p_fox *(0 - yard) + (1 - p_fox) * 1
#
#    if not win_gather:
#        return 'wait'
#    if not win_wait:
#        return 'gather'
#    if win_gather >= win_wait:
#        return 'gather'
#    else:
#        return 'wait'
