...
def poly(coefs):
    ...
    def fn(x):
        p = 0
        for index, v in enumerate(coefs):
            p = v * x**index + p
        return p
    ...
    for index, v in enumerate(coefs):
        if v is not 0:
            if index == 0:
                ...
                if v == 1:
                    name.append('x**' + str(index))
                else:
                    name.append(str(v) +' * x**' + str(index))
    name.reverse()
    fn.__name__ =  ' + '.join(name)
    fn.coefs = coefs
    return fn
...
def add(p1, p2):
    ...
    c1 = p1.coefs
    c2 = p2.coefs
    coefs = [0] * (max(len(c1), len(c2)))
    for index in xrange(len(coefs)):
        if index < len(c1):
            coefs[index] += c1[index]
        if index < len(c2):
            coefs[index] += c2[index]
    coefs = tuple(coefs)
    return poly(coefs)
...
def mul(p1, p2):
    ...
    c1 = p1.coefs
    c2 = p2.coefs
    p3 = [0] * (len(c1) + len(c2) - 1)
    for ai, a in enumerate(c1):
        for bi, b in enumerate(c2):
            p3[ai+bi] += a*b
...
def deriv(p):
    ...
    coefs = p.coefs
    d = []
    for index, a in enumerate(coefs):
        if index != 0:
            d.append(a * index)
    return poly(tuple(d))
